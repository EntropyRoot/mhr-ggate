...
class FrontedGASTransport(Transport):
    ...
    async def _read_chunked(self, reader: asyncio.StreamReader, buf: bytes) -> bytes:
        """Read an HTTP/1.1 ``Transfer-Encoding: chunked`` body (RFC 7230).

        This is stricter than the upstream lenient version because we keep
        the TLS connection in a pool and reuse it for the next request: any
        leftover bytes after the body (chunk extensions, trailers, the
        terminating CRLF) MUST be drained so the next response we read
        does not start mid-stream.

        Raises:
          RuntimeError on a malformed chunk size, a missing chunk-data
          CRLF, an over-cap body, or premature EOF. We never return a
          partial body — for a tunnel transport, partial == corrupt.
        """
        out = bytearray()
        ...
        async def _need_bytes(n: int) -> bytes:
            """Pull exactly *n* bytes from buf (filling from reader)."""
            nonlocal buf
            while len(buf) < n:
                more = await asyncio.wait_for(reader.read(min(65536, n - len(buf))),
                                              timeout=20.0)
                if not more:
                    raise RuntimeError(
        ...
        while True:
            ...
            # Chunk size syntax: HEXDIG+ ( ";" chunk-ext )* CRLF
            # We split off any extension and ignore it, but a malformed
            # size is fatal — the stream framing is no longer trustworthy.
            head = line.split(b";", 1)[0].strip()
            if not head:
                # Some implementations send a stray blank line between
                # chunks; treat that as malformed rather than silently
                # skipping, because a true blank line should have been
                # the terminator after a chunk's data, not before a size.
                raise RuntimeError("chunked: empty chunk size line")
            try:
                size = int(head, 16)
            except ValueError as exc:
                raise RuntimeError(
                    f"chunked: bad chunk size {head!r}"
                ) from exc
            if size < 0:
                raise RuntimeError(f"chunked: negative chunk size {size}")
            if size == 0:
                # Drain optional trailer headers, terminated by an empty line.
                while True:
            ...
            if size > cap or len(out) + size > cap:
                raise RuntimeError(
                    f"chunked: response exceeds cap of {cap} bytes"
                )
            chunk = await _need_bytes(size)
            out.extend(chunk)
            # RFC 7230 §4.1: each chunk-data is followed by CRLF.
